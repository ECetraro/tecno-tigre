...
// Data Management - AHORA USA API.PHP
async function getLocalPosts() {
    try {
        const response = await fetch('api.php');
        const data = await response.json();

        // Si no hay datos en el servidor, cargar iniciales de data.js
        if (!data || data.length === 0) {
            console.log("No hay posts en el servidor, usando iniciales...");
            return initialPosts;
        }
        return data;
    } catch (e) {
        console.error("Error al cargar posts desde el servidor:", e);
...
if (postForm) {
    postForm.addEventListener('submit', async (e) => {
        ...
        // GUARDAR EN EL SERVIDOR VIA API.PHP
        try {
            const response = await fetch('api.php', {
                method: 'POST',
            ...
            if (response.ok) {
                showSection('list');
                loadPosts();
                alert('¡Publicación guardada con éxito en el servidor!');
            } else {
